...
pub fn vote_pos_nft(ctx: Context<VoteNFT>) -> Result<()> {
    ...
    match voter.num_votes {
        NumVotes::Three => {
            ...
            msg!("{}", ctx.accounts.voter_edition_mark.key());
            msg!("{}", ctx.accounts.voter_edition_account.key());
            msg!("{}", ctx.accounts.voter_metadata_account.key());
            msg!("{}", ctx.accounts.voter_mint.key());
            msg!("{}", ctx.accounts.voter_token_account.key());
